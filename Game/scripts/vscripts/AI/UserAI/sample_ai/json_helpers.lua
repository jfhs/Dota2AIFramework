...
function Dota2AI:JSONChat(event)	
	...
	return dkjson.encode(jsonEvent)
end
 ...
 function Dota2AI:JSONunit(eUnit)	
	...
	unit.attackAnimationPoint = eUnit:GetAttackAnimationPoint()
--	unit.currentAttackAnimationPoint = eUnit:GetAttackPoint()
	unit.isAttacking = eUnit:IsAttacking()
	unit.id = eUnit:GetEntityIndex()

	unit.name = eUnit:GetName()
	unit.team = eUnit:GetTeamNumber()
	unit.attackRange = eUnit:GetAttackRange()
	unit.damage = eUnit:GetAverageTrueDamage()
	unit.armor = eUnit:GetPhysicalArmorValue()
	...
	if eUnit:IsHero() then
		...
		unit.lasthits = eUnit:GetLastHits()
		unit.xp = eUnit:GetCurrentXP()
	...
	if attackTarget then
		unit.attackTarget = attackTarget:GetEntityIndex()
	end
...
--	end
	...
	local allUnits = AI_FindUnitsInRadius(eHero:GetOrigin(),
	...
	for _,unit in pairs(allUnits) do
		world.entities[unit:GetEntityIndex()]=self:JSONunit(unit)
	end
...
--	buildings[35] =  Entities:FindByName(nil, "ent_dota_fountain_bad")
	...
	world.myHeroId = eHero:GetEntityIndex()
	world.gameTime = AI_GetGameTime()
	
	return dkjson.encode(world)
end

function Dota2AI:JSONEndGame()
	local lasthits = {}
	local localPlayer = 0 -- PlayerResource:GetLocalPlayer()
	lasthits[localPlayer] = PlayerResource:GetLastHits(localPlayer)

	local stats = {}
	stats["lasthits"] = lasthits
