...
from libs.packet_prnt import PacketPRNT
from libs.packet_ainf import PacketAINF
from libs.packet_adon import PacketADON
from libs.packet_chan import PacketCHAN
from libs.packet_cvar import PacketCVAR
from libs.packet_cfgv import PacketCFGV
...
class VConsole2Lib:
    ...
    def send_cmd(self, cmd):
        cmd_array = bytearray()
        cmd_array.extend(bytearray("CMND", encoding='ascii'))
        cmd_array.extend(bytearray([0x00, 0xD3, 0x00, 0x00]))
        ...
        cmd_array.extend(bytearray(cmd, encoding='ascii'))
    ...
    def __listen(self):
        print("Self on_prnt_received is " + str(self.on_prnt_received))
        ...
        try:
            while 1:
                ...
                #print("Got type " + self.stream.msg_type)

                if self.stream.msg_type == 'PRNT':
                    prnt = PacketPRNT(self.stream)
                    if channel is None:
                        #print("We dont have channel, but calling on_prnt_received")
                        if self.on_prnt_received:
                            self.on_prnt_received(self, "UNKNOWN", prnt.msg)
                        continue
